...
def simulation_without_antibiotic(num_bacteria,
    ...
    for i in range(num_trials):
        bacteria = [SimpleBacteria(birth_prob, death_prob) for j in range(num_bacteria)]
    ...
    x_coords = range(301)
    y_coords = [calc_pop_avg(all_trials_data, i) for i in x_coords]
    make_one_curve_plot(x_coords, y_coords, "Time", "Average Population", "Simulation of SimpleBacteria")
...
class TreatedPatient(Patient):
    ...
    def update(self):
        ...
        if self.on_antibiotic:
            survivors = [b for b in survivors if b.get_resistant()]
        pop_density = len(survivors) / self.max_pop
        offspring = []
        for i in survivors:
            try:
...
def simulation_with_antibiotic(num_bacteria,
    ...
    all_resistant_data = []
    for i in range(num_trials):
        bacteria = [ResistantBacteria(birth_prob, death_prob, resistant, mut_prob) for j in range(num_bacteria)]
        patient = TreatedPatient(bacteria, max_pop)
        trial_data = [patient.get_total_pop()]
        resistant_data = [patient.get_resist_pop()]
        for j in range(150):
            patient.update()
            trial_data.append(patient.get_total_pop())
            resistant_data.append(patient.get_resist_pop())
        ...
        for j in range(250):
            ...
            resistant_data.append(patient.get_resist_pop())
        all_trials_data.append(trial_data)
        all_resistant_data.append(resistant_data)
    x_coords = range(401)
    y_coords1 = [calc_pop_avg(all_trials_data, i) for i in x_coords]
    y_coords2 = [calc_pop_avg(all_resistant_data, i) for i in x_coords]
    make_two_curve_plot(x_coords, y_coords1, y_coords2, "Total", "Resistant", "Timestep", "Average Population" , "Simulation with Antibiotic" )
    return (all_trials_data, all_resistant_data)
